...
метод `async start()`. Если этот метод есть в сервисе, значит сервис можно вызвать из командной строки.

Главный файл `index.js` может выглядеть следующим образом:
